"use client";
import { useCreateAdminForm } from "@/hooks/super-admin/useCreateAdminForm";
...
export default function AddAdminModal() {
  ...
  const { register, errors, isPending, onSubmit } = useCreateAdminForm(() =>
    setOpen(false),
  );
      ...
      <Dialog.Portal>
        ...
        >
          <div className="flex items-center justify-between pb-5 border-b border-border shrink-0">
            <Dialog.Title className="title">Add New Admin</Dialog.Title>
          ...
          <form onSubmit={onSubmit} noValidate className="flex flex-col flex-1 min-h-0">
            <div className="w-full my-6 space-y-5 overflow-y-auto flex-1 pr-2">
              <AddAdminForm register={register} errors={errors} />
            </div>
