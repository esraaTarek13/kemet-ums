...
import { useUpdateStudentProfileForm } from "@/hooks/admin/students/useUpdateStudentProfileForm";

interface UpdateStudentProfileModalProps {
  studentId: string;
}

export default function UpdateStudentProfileModal({
  studentId,
}: UpdateStudentProfileModalProps) {
  const [open, setOpen] = useState(false);
  const { register, control, errors, isPending, onSubmit, departmentOptions } =
    useUpdateStudentProfileForm({
      studentId,
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger asChild>
        <button
          type="button"
          className="btn btn-dark w-full xl:w-auto flex items-center justify-center gap-1.5 py-2"
        >
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 card-top-border w-[90%] max-w-lg max-h-[90vh] flex flex-col">
          <div className="flex items-center justify-between pb-5 border-b border-border shrink-0">
            <Dialog.Title className="title">Edit Student Details</Dialog.Title>
            {/* a11y: real description instead of suppressing the Radix warning */}
            <Dialog.Description className="sr-only">
              Update this student&apos;s profile information
            </Dialog.Description>
